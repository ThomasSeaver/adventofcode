...
for (const code of codeList) {
  ...
  let minimumSequenceLength = 0;
  for (let passIndex = 0; passIndex < 25; passIndex += 1) {
    logDebug(passIndex, sequenceList.length);
    ...
    minimumSequenceLength = sequenceList.reduce(
    ...
    sequenceList = sequenceList.filter(
      (sequence) => sequence.length === minimumSequenceLength
    );

    let minimumSequenceChanges = Number.MAX_SAFE_INTEGER;
    for (const sequence of sequenceList) {
      const sequenceList = sequence.split("");
      const sequenceChangeCount = sequenceList
        .map(
          (key, keyIndex) =>
            (keyIndex == sequenceList.length - 1
              ? 0
              : directionalKeypadPathMap[key][sequenceList[keyIndex + 1]]
                  .length) as number
        )
        .reduce((previous, current) => previous + current, 0);
      minimumSequenceChanges = Math.min(
        sequenceChangeCount,
        minimumSequenceChanges
      );
    }
    sequenceList = sequenceList.filter((sequence) => {
      const sequenceList = sequence.split("");
      const sequenceChangeCount = sequenceList
        .map(
          (key, keyIndex) =>
        ...
        .reduce((previous, current) => previous + current, 0);
      return minimumSequenceChanges === sequenceChangeCount;
    });
  }

  sum += minimumSequenceLength * parseInt(code.slice(0, code.length - 1));
}
